...
XRay.prototype.getMaxXYZ = function(){
	...
	if(!self.mflag){
	    self.mflag = true;
	    self.minx = self.maxx = point.x;
	    self.miny = self.maxy = point.y;
	    self.minz = self.maxz = point.z;
	}
	else{
    ...
    self.maxp = Math.max(self.maxx, self.maxy, self.maxz);
    self.minp = Math.min(self.minx, self.miny, self.minz);

    self.dx = self.maxx - self.minx;
    self.dy = self.maxy - self.miny;
    self.dz = self.maxz - self.minz;
    self.dp = self.maxp - self.minp;
...
XRay.prototype.initThree = function(){
	...
	for(ly=0;ly<2;ly++){
		...
		this.light.position.set(larr[lx]*this.maxx*this.mscale, larr[ly]*this.maxy*this.mscale, larr[lz]*this.maxz*this.mscale);
